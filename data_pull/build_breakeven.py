...
BREAKEVEN_TENORS = ("5-Year", "10-Year", "30-Year")
...
def incremental_breakeven(conn) -> int:
    ...
    date_filter = "AND n.ts > %(max_date)s"
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            {sql}
            """,
            {"tenors": BREAKEVEN_TENORS, "max_date": max_breakeven},
        )
        row_count = cur.rowcount
    conn.commit()
    return row_count
