...
from tests.synthetic import research_panel
...
def test_concrete_10s30s_research_reports_run_on_the_shared_synthetic_panel():
    for name in MODULES:
        ...
        state = module.run(research_panel(n=900))
...
def test_rv_and_fair_value_reports_include_their_method_specific_evidence():
    ...
    rv_state = rv.run(research_panel(n=900))
    fair_state = fair.run(research_panel(n=900))
