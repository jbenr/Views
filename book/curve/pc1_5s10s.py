"""5s10s vs PC1 of the yield panel - the curve hedged on the level factor.

Question: is the 5s10s curve rich or cheap versus the level factor of the
...
PC1 is point-in-time and sign-fixed over a trailing 126-day window, so the
...
PC1_LB provenance: xy_scan's winning PCA window for 5s10s. It is a tuning
...
configuration plus the PC1 feature hook. Same funnel as every curve strategy:
    ...
    python -m book.curve.pc1_5s10s              # single run, live DB
    python -m book.curve.pc1_5s10s --predict    # setup search
    python -m book.curve.pc1_5s10s --exit       # exits per saved setup
    python -m book.curve.pc1_5s10s --sweep      # exact engine + trade logs
    python -m book.curve.pc1_5s10s --cook       # all three, in order
"""
...
SIGNAL_NAME = "pc1_5s10s"

PC1_COLS = ['2y', '5y', '10y', '30y']
PC1_LB = 126  # xy_scan's winning PCA window for 5s10s
...
STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.pc1_5s10s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/pc1_5s10s/
    tickers=TICKERS,
