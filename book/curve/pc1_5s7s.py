"""5s7s vs PC1 of the yield panel - the curve hedged on the level factor.

Question: is the 5s7s curve rich or cheap versus the level factor of the
...
configuration plus the PC1 feature hook. Same funnel as every curve strategy:
    ...
    python -m book.curve.pc1_5s7s              # single run, live DB
    python -m book.curve.pc1_5s7s --predict    # setup search
    python -m book.curve.pc1_5s7s --exit       # exits per saved setup
    python -m book.curve.pc1_5s7s --sweep      # exact engine + trade logs
    python -m book.curve.pc1_5s7s --cook       # all three, in order
"""
...
STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.pc1_5s7s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/pc1_5s7s/
    tickers=TICKERS,
