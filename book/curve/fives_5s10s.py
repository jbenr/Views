"""5y vs 5s10s - the curve hedged against its own short leg.

Question: is the 5s10s curve rich or cheap versus the level of 5Y?

Model:
    y = TARGET = 5s10s curve
    x = FEATURE = 5Y yield
...
Directions are in CURVE space: positive residual means 5s10s is steep/rich
vs 5Y -> short 5s10s; negative means flat/cheap -> long 5s10s.

Futures expression: FV (short leg) vs UXY (long leg).
...
configuration. Tune by overriding Strategy fields below.
    ...
    python -m book.curve.fives_5s10s              # single run, live DB
    python -m book.curve.fives_5s10s --synthetic  # single run, no DB
    python -m book.curve.fives_5s10s --predict    # setup search -> setups parquet
    python -m book.curve.fives_5s10s --exit       # exits per setup -> exits parquet
    python -m book.curve.fives_5s10s --sweep      # exact engine + trade logs
    python -m book.curve.fives_5s10s --cook       # all three, in order
...
SIGNAL_NAME = "fives_5s10s"

TICKERS = {
    "5y": "USGG5YR Index",  # % -> scaled to bps at load
    "5s10s": "USYC5Y10 Index",  # already quoted in bps
}
...
def synthetic_data(n: int = 1500, seed: int = 36):
    """Synthetic substitute: 5s10s explained by 5Y plus an OU residual."""
    return synthetic_pair(TARGET, FEATURE, n=n, seed=seed)


STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.fives_5s10s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/fives_5s10s/
    tickers=TICKERS,
    bps_cols=["5y"],
    target=TARGET,
    feature=FEATURE,
