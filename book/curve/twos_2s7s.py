"""2y vs 2s7s - the curve hedged against its own short leg.

Question: is the 2s7s curve rich or cheap versus the level of 2Y?
...
Directions are in CURVE space: positive residual means 2s7s is steep/rich
vs 2Y -> short 2s7s; negative means flat/cheap -> long 2s7s.

Futures expression: TU (short leg) vs TY (long leg).
...
configuration. Tune by overriding Strategy fields below.
    ...
    python -m book.curve.twos_2s7s --cook       # all three, in order
...
STRATEGY = Strategy(
    ...
    path=Path(__file__),  # funnel artifacts live in data/twos_2s7s/
    tickers=TICKERS,
    bps_cols=["2y"],
    target=TARGET,
    feature=FEATURE,
