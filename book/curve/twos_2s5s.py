"""2y vs 2s5s - the curve hedged against its own short leg.

Question: is the 2s5s curve rich or cheap versus the level of 2Y?
...
Directions are in CURVE space: positive residual means 2s5s is steep/rich
vs 2Y -> short 2s5s; negative means flat/cheap -> long 2s5s.

Futures expression: TU (short leg) vs FV (long leg).
...
configuration. Tune by overriding Strategy fields below.
    ...
    python -m book.curve.twos_2s5s              # single run, live DB
    python -m book.curve.twos_2s5s --synthetic  # single run, no DB
    python -m book.curve.twos_2s5s --predict    # setup search -> setups parquet
    python -m book.curve.twos_2s5s --exit       # exits per setup -> exits parquet
    python -m book.curve.twos_2s5s --sweep      # exact engine + trade logs
    python -m book.curve.twos_2s5s --cook       # all three, in order
...
def synthetic_data(n: int = 1500, seed: int = 31):
    """Synthetic substitute: 2s5s explained by 2Y plus an OU residual."""
    return synthetic_pair(TARGET, FEATURE, n=n, seed=seed)


STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.twos_2s5s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/twos_2s5s/
    tickers=TICKERS,
    bps_cols=["2y"],
    target=TARGET,
    feature=FEATURE,
