"""5s30s vs PC1 of the yield panel - the curve hedged on the level factor.

Question: is the 5s30s curve rich or cheap versus the level factor of the
...
PC1_LB provenance: xy_scan's winning PCA window for 5s30s. It is a tuning
...
configuration plus the PC1 feature hook. Same funnel as every curve strategy:
    ...
    python -m book.curve.pc1_5s30s              # single run, live DB
    python -m book.curve.pc1_5s30s --predict    # setup search
    python -m book.curve.pc1_5s30s --exit       # exits per saved setup
    python -m book.curve.pc1_5s30s --sweep      # exact engine + trade logs
    python -m book.curve.pc1_5s30s --cook       # all three, in order
"""
...
SIGNAL_NAME = "pc1_5s30s"

PC1_COLS = ['2y', '5y', '10y', '30y']
PC1_LB = 252  # xy_scan's winning PCA window for 5s30s
...
STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.pc1_5s30s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/pc1_5s30s/
    tickers=TICKERS,
