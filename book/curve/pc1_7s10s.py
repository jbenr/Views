"""7s10s vs PC1 of the yield panel - the curve hedged on the level factor.

Question: is the 7s10s curve rich or cheap versus the level factor of the
...
configuration plus the PC1 feature hook. Same funnel as every curve strategy:
    ...
    python -m book.curve.pc1_7s10s              # single run, live DB
    python -m book.curve.pc1_7s10s --predict    # setup search
    python -m book.curve.pc1_7s10s --exit       # exits per saved setup
    python -m book.curve.pc1_7s10s --sweep      # exact engine + trade logs
    python -m book.curve.pc1_7s10s --cook       # all three, in order
"""
...
STRATEGY = Strategy(
    name=SIGNAL_NAME,
    module="book.curve.pc1_7s10s",  # sweep workers import this
    path=Path(__file__),  # funnel artifacts live in data/pc1_7s10s/
    tickers=TICKERS,
